...
void main()
{
  ...
  if(mat.illum == 3){
	...
	vec3 reflectedRayDir = reflect(gl_WorldRayDirectionEXT, normal);
	createCoordinateSystem(reflectedRayDir, tangent, bitangent);
	rayDirection = samplingPhongDistribution(prd.seed, pushC.sphereGlossiness, tangent, bitangent, reflectedRayDir, p, cos_theta);

   }  else if(mat.illum == 2){
	
		vec3 refractedRayDir = refract(gl_WorldRayDirectionEXT, normal, mat.ior);
		createCoordinateSystem(refractedRayDir, tangent, bitangent);
		rayDirection = samplingPhongDistribution(prd.seed, pushC.sphereGlossiness, tangent, bitangent, refractedRayDir, p, cos_theta);
	
  }else{
  
    createCoordinateSystem(normal, tangent, bitangent);
	rayDirection = samplingHemisphere(prd.seed, tangent, bitangent, normal, p);
